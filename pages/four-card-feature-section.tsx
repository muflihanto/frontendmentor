...
const FourCardFeature = () => {
  return (
    <>
      ...
      <div className="App font-poppins bg-four-card-neutral-100 relative flex min-h-[100svh] items-center justify-center pt-[84px] pb-[78px]">
        <Main />
        <Footer />
...
function Main() {
  ...
  return (
    <div className="flex max-w-[375px] flex-col items-center px-8 lg:max-w-none">
      <div className="text-four-card-neutral-200">
        <h1 className="text-four-card-neutral-300 text-center text-[24px] font-extralight tracking-[0.2px]">
          Reliable, efficient delivery
          <span className="font-semibold"> Powered by Technology</span>
        </h1>
        <p className="mt-4 text-center text-[15px] leading-[25px] tracking-[0.1px]">Our Artificial Intelligence powered tools use millions of project data points to ensure that your project is successful</p>
      </div>
      <div className="mt-[75px] max-lg:flex max-lg:flex-col max-lg:gap-y-[24px] lg:grid lg:grid-cols-[repeat(3,minmax(0px,350px))] lg:grid-rows-[repeat(4,minmax(0px,125px))] lg:gap-x-[28px] lg:gap-y-[30px]">
...
const Card: FC<CardProps> & CardSubComponents = ({ className, children }) => {
  return <div className={`relative flex h-[222px] flex-col overflow-hidden rounded-lg bg-white px-[28px] pt-[31px] pb-[28px] shadow-xl shadow-[hsla(180,51%,29%,0.13)] before:absolute before:top-0 before:left-0 before:h-1 before:w-full lg:row-span-2 lg:h-[250px] ${className}`}>{children}</div>;
};

const Heading: React.FC<{ children: ReactNode }> = ({ children }) => {
  return <h2 className="text-four-card-neutral-300 text-[20px] font-bold leading-none tracking-[-0.2px]">{children}</h2>;
};
Card.Heading = Heading;

const Body: React.FC<{ children: ReactNode }> = ({ children }) => {
  return <p className="text-four-card-neutral-200 mt-2 px-[1px] text-[13px] leading-[23px] tracking-[0.1px]">{children}</p>;
};
Card.Body = Body;
...
const Icon: FC<{ variant: CardVariant }> = ({ variant }) => {
  return (
    <svg
      ...
      viewBox="0 0 64 64"
      className="mt-auto h-[57px] self-end"
    >
      {icons[variant]}
    </svg>
  );
