...
export default function About(props: AboutProps) {
  return (
    <Card className="mt-6 px-[23px] pb-[39px] pt-[38px] lg:px-12 lg:pb-[47px] lg:pt-[45px]">
      <h2 className="text-[18px] font-bold lg:text-[20px]">About this project</h2>
      <p className="text-crowdfunding-neutral-100/75 mt-[22px] text-[14px] font-medium leading-[24px] lg:mt-[33px] lg:text-[16px] lg:leading-[30px]">
      ...
      <p className="text-crowdfunding-neutral-100/75 mt-[25px] text-[14px] font-medium leading-[24px] -tracking-[0.01px] lg:mt-[30px] lg:text-[16px] lg:leading-[30px]">Featuring artisan craftsmanship, the simplicity of design creates extra desk space below your computer to allow notepads, pens, and USB sticks to be stored under the stand.</p>
...
function Reward({ data, value, openSelectionModal }: RewardProps) {
  ...
  return (
    <div className={`rounded-lg border px-[23px] pb-[23px] pt-[21px] lg:px-[31px] lg:pb-[31px] lg:pt-[34px] ${stock === 0 && "select-none opacity-50"}`}>
      <div className="lg:flex lg:justify-between">
        <h3 className="text-[14px] font-bold lg:text-[18px]">{name}</h3>
        <p className="text-crowdfunding-primary-100 mt-1 text-[14px] font-medium lg:mt-[3px] lg:text-[15px]">Pledge ${startsFrom} or more</p>
      </div>
      <p className="text-crowdfunding-neutral-100/80 mt-[23px] text-[14px] font-medium leading-[24px] lg:mt-[21px] lg:text-[16px] lg:leading-[30px]">{details}</p>
      <div className="lg:mt-[25px] lg:flex lg:items-center lg:justify-between">
        ...
        <button
          className={`mt-[19px] h-[48px] w-[157px] rounded-full text-[14px] font-bold text-white/90 lg:m-0 ${stock === 0 ? "bg-crowdfunding-neutral-100/75 cursor-default" : "bg-crowdfunding-primary-100 hover:bg-crowdfunding-primary-200"}`}
          value={value}
